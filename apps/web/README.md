...
- ADMIN 默认进入 `/admin/dashboard`；REVIEWER 默认进入 `/admin/review`。审核入口以 6 秒自动轮播展示任务整体进度和审核处理进度，常驻显示积压、待领取、审核中和今日完成；箭头、圆点或超过 48px 的拖动会锁定当前页直到离开入口，减少动态效果设置下不自动播放。
...
- 采集权限页使用等宽双栏，小程序用户默认每页 10 条并预留 10 行高度，待审批申请和已授权用户默认每页 5 条并各预留 5 行高度；三块分页固定贴底，空数据、少于一页和窄屏单列时保持卡片高度稳定。
