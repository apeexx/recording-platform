...
- 审核池同时展示 `SUBMITTED`（已提交）与 `REVIEW_PENDING`（待审核），并将采集员、审核员的用户 ID 与姓名拆列显示。混合勾选后，“批量领取审核”和“批量分配”只处理已提交记录，“批量通过”只处理待审核记录；管理员分配审核员使用姓名、用户 ID 或登录账号搜索选择。审核释放后回到已提交，批量结果按成功、失败和状态变化跳过汇总提示。
