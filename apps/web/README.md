...
当前不使用 JWT 或 Pinia。任务配置、数据池/CSV 导入、采集权限、审核池/工作台、后台用户、操作记录和统计页面已接入真实接口；通用下拉框、胶囊开关和 1–600 秒双端时长滑块均为无依赖组件。表单校验、操作、搜索、刷新及接口失败统一使用右上角 Toast：错误默认约 4.5 秒并按相同文案去重，成功和信息默认约 2.6 秒；操作失败保留当前页面或弹窗输入，已有数据刷新失败保留旧数据。只有核心数据首次加载失败使用 `AsyncState` 重试状态，账号接管和危险操作仍使用确认弹窗；业务表单通过 `novalidate` 与手动校验避免浏览器原生提示风格不一致。
