...
- ADMIN 默认进入 `/admin/dashboard`；REVIEWER 默认进入 `/admin/review`。审核入口请求包含已清空的人工审核任务，用全部返回项计算真实双进度和积压/待领取/审核中/今日完成四项指标；审核处理进度只以当前 `SUBMITTED/REVIEW_PENDING/REWORK_PENDING/COMPLETED` 为分母，已完成与返修中为已处理。仅把仍有积压的任务展示为待办卡，卡片首行同行展示编号与名称，下一行并排展示任务完成度和审核处理进度；积压全部处理完成后继续保留概览并显示完成态。桌面端轮播在左、指标以 2×2 放在右侧，窄屏自动上下排列；轮播每 6 秒切换，箭头、圆点或超过 48px 的拖动会锁定当前页直到离开入口，减少动态效果设置下不自动播放。轮播文字不可选择，拖动时不会误选；双进度、四项指标、AI 设置字段和任务参考源字段均使用可悬浮、聚焦及点击固定的通用问号说明。
- 审核池同时展示 `SUBMITTED`（已提交）与 `REVIEW_PENDING`（待审核），顶部展示任务四项指标，五维筛选独立成工具栏，选中条目后再显示跨页批量操作条，并将同一筛选快照用于跨页批处理。审核工作台左侧吸顶展示参考源，右侧依次展示只读原始采集结果、AI 候选、独立最终答案和审核结论；当前领取审核员或管理员可在顶部将确认无效的条目标记为废弃，保留采集结果和审核归属供管理员恢复；视频使用固定画布与 `object-fit: contain`。
...
- 浏览器 favicon、后台登录页和管理端左上品牌标识统一使用与小程序字节一致的“砚数声采”SVG，Web 副本位于 `public/assets/branding/yanshu-avatar.svg`；页面标题固定为“录音任务平台”。
- 任务配置、数据池/CSV 导入、权限、审核、用户、操作记录和统计均使用真实 API；任务仅在草稿状态显示编辑入口，运行中仅允许暂停，暂停后才显示恢复和结束；发布后定义冻结；平台板块已整体移除。
- 工作统计收敛为管理员可见的“采集员统计”，旧 `/admin/reports/tasks` 重定向至数据大屏，审核统计入口、页面和 API 已移除。采集员统计必须选择任务，默认当前业务日，并提供今天、昨天、近 7 日、本月、全部和自定义范围；独立人员详情复用同一套快捷日期与范围选择，无 URL 日期时保持“全部”，切换后自动重置提交/完成明细页码并刷新。业务日统一为 Asia/Shanghai 04:00 至次日 04:00，凌晨 0–3 点仍属于前一业务日。任务选择和日期变化会立即查询。日期范围在选择开始日后随悬停实时预览，表头按降序、升序、默认三态循环；刷新保留旧数据并显示圆形加载层。24 小时首次提交分布仍展示真实北京时间小时，但顺序为 04→03。任务、日期、显式排序、页码和每页条数通过 URL 恢复，详情 CSV 继承当前日期。
- 通用 `HelpPopover` 使用跨实例唯一 ID，同一时间只固定一个说明；悬浮、键盘聚焦、点击固定、外部点击和 Esc 均可操作。生产路由的关键复杂规则已覆盖数据大屏、任务/数据池、条目/权限、审核、统计、系统管理与三类语音生成页面，说明文案不包含密钥、完整媒体地址或内部异常。审核入口顶部公式说明保留，小任务卡移除重复问号，轮播标题使用左右等宽占位严格居中。
- 任务详情为双栏工作台：草稿、运行中和暂停任务均可立即单条添加或导入数据，结束任务拒绝新增。CSV 显式开始后使用稳定任务 ID 每秒刷新真实进度，完成时 Toast 并自动刷新；草稿任务支持安全级联删除。
- 后台数据列表统一使用包含总数、每页条数、首尾页、连续页码、省略号及前后箭头的数字分页。任务详情数据池默认 10 条，其余普通列表默认 20 条并可选 10/20/50；操作记录默认 50 条并可选 20/50/100，采集权限保留 5/10。用户、邀请码、操作记录和语音生成记录均使用真实服务端分页。独立任务数据池把筛选、页码和每页条数写入当前 URL，查看详情后返回可恢复原状态；从其他菜单重新进入时重置临时状态，但浏览器 `localStorage` 继续恢复最后选择的任务。数据池继续支持服务端筛选、脚本来源展示、已授权采集员搜索和全量中文 CSV 导出。
- 任务详情数据池、独立任务数据池和任务审核池均支持单条、本页全选与“选择全部筛选结果”。独立任务数据池可将所选 `AVAILABLE` 数据分配给当前任务已有 ACTIVE 授权且账号 ACTIVE 的采集员，本页与跨页执行均保留 revision/CAS、幂等和恢复能力；已分配数据必须先释放再改派。两个任务数据池按状态提供“查看、释放、废弃、恢复”行内快捷操作，批量入口仍保留；跨页预览、快照和执行携带相同筛选条件，避免处理不可见数据。
- 审核入口、独立数据池批处理和任务详情 CSV 导入的可见百分比统一固定显示两位小数；进度条继续使用未格式化数值，分母为零显示 `0.00%`。
