...
- 审核池同时展示 `SUBMITTED`（已提交）与 `REVIEW_PENDING`（待审核），并将采集员、审核员的用户 ID 与姓名拆列显示。混合勾选后，“批量领取审核”和“批量分配”只处理已提交记录，“批量通过”只处理待审核记录；管理员分配审核员使用姓名、用户 ID 或登录账号搜索选择。审核工作台固定展示参考文本、参考音频、参考视频，URL 优先、历史媒体 ID 回退，缺失项显示“无”。
...
- 数据池使用紧凑数字服务端分页，支持每页 5/10/20 条并默认 10 条；每页条数固定在左侧且菜单向上展开，页码和翻页固定在右侧，窄屏自然换行。独立“任务数据池”页面继续保持每页 20 条。
- 任务详情数据池、独立任务数据池和任务审核池均支持单条、本页全选与“选择全部筛选结果”。本页模式切页清空，跨页模式保留排除项；跨页操作固化条目 revision 快照并进入可恢复异步任务，页面显示成功、失败、状态变化跳过及最近任务进度。
