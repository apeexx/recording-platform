...
- 工作统计收敛为管理员可见的“采集员统计”，旧 `/admin/reports/tasks` 重定向至数据大屏，审核统计入口、页面和 API 已移除。采集员统计必须选择任务，默认当天，并提供今天、昨天、近 7 日、本月、全部和自定义范围；任务内总表支持五项指标排序，点击人员后在同页展示五项汇总、每日统计、最近提交及分页完整明细。
- 任务详情为双栏工作台：草稿、运行中和暂停任务均可立即单条添加或导入数据，结束任务拒绝新增。CSV 显式开始后使用稳定任务 ID 每秒刷新真实进度，完成时 Toast 并自动刷新；草稿任务支持安全级联删除。
- 数据池使用服务端分页，编号、脚本来源 ID 前缀、四组状态、单个或多个采集员、未分配采集员及结果内容均为 Teleport 高层级悬浮筛选菜单，不改变表格尺寸且不受滚动容器裁剪；来源列显示 sourcePlatform 和可复制 sourceItemId，无绑定时显示“平台内创建”。任务详情支持每页 5/10/20 条并默认 10 条；独立“任务数据池”保持每页 20 条。两个数据池和采集员统计都可导出继承当前筛选的全量 UTF-8 BOM CSV：先经统一请求链预检会话、权限、任务和日期，再由浏览器原生下载，不受页面分页限制，也不在前端内存中先聚合完整文件。
