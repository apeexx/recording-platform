...
- 任务详情为双栏工作台：左侧分开放置添加与批量导入卡片，右侧保留宽表格。CSV 可拖入或点击选择，显式开始后通过独立稳定的导入任务 ID 每秒刷新后端逐行 checkpoint 的总数/已处理/成功/失败和真实进度；创建响应使用 `importJobId`，完整状态响应使用 `id`，前端不会再因字段差异中断后续轮询。快速任务可直接显示 100%，完成时 Toast 并自动刷新；提供带 BOM 的固定三列示例下载。单条添加、CSV 和待领取编辑的音视频均只提交 HTTPS URL，不上传或下载参考媒体；待领取条目支持弹窗编辑和真正删除，其他状态仍需先释放；草稿任务支持删除。
- 数据池使用紧凑数字服务端分页，支持每页 5/10/20 条并默认 10 条，显示首尾页、省略号和左右箭头；翻页或切换条数会清空当前页勾选，独立“任务数据池”页面继续保持每页 20 条。
...
当前不使用 JWT 或 Pinia。任务配置、数据池/CSV 导入、采集权限、审核池/工作台、后台用户、操作记录和统计页面已接入真实接口；通用下拉框、胶囊开关和 1–600 秒双端时长滑块均为无依赖组件。表单校验、操作、搜索、刷新及接口失败统一使用右上角 Toast：错误默认约 4.5 秒并按相同文案去重，使用不依赖系统字体的本地描边 SVG 图标；成功和信息默认约 2.6 秒。操作失败保留当前页面或弹窗输入，已有数据刷新失败保留旧数据。只有核心数据首次加载失败使用 `AsyncState` 重试状态，账号接管和危险操作仍使用确认弹窗；业务表单通过 `novalidate` 与手动校验避免浏览器原生提示风格不一致。
