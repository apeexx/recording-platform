...
- 后台数据列表统一使用包含总数、每页条数、首尾页、连续页码、省略号及前后箭头的数字分页。任务详情数据池默认 10 条，其余普通列表默认 20 条并可选 10/20/50；操作记录默认 50 条并可选 20/50/100，采集权限保留 5/10。用户、邀请码、操作记录和语音生成记录均使用真实服务端分页。独立任务数据池把筛选、页码和每页条数写入当前 URL，查看详情后返回可恢复原状态；从其他菜单重新进入时重置临时状态，但浏览器 `localStorage` 继续恢复最后选择的任务。数据池继续支持服务端筛选、脚本来源展示和全量中文 CSV 导出。
