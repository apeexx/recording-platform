...
- 工作统计收敛为管理员可见的“采集员统计”，旧 `/admin/reports/tasks` 重定向至数据大屏，审核统计入口、页面和 API 已移除。采集员统计必须选择任务，默认当前业务日，并提供今天、昨天、近 7 日、本月、全部和自定义范围；业务日统一为 Asia/Shanghai 04:00 至次日 04:00，凌晨 0–3 点仍属于前一业务日。任务选择和日期变化会立即查询。日期范围在选择开始日后随悬停实时预览，表头按降序、升序、默认三态循环；刷新保留旧数据并显示圆形加载层。24 小时首次提交分布仍展示真实北京时间小时，但顺序为 04→03。点击人员进入独立详情页，任务、日期、显式排序、页码和每页条数通过 URL 恢复。
