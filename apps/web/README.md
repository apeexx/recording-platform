...
- ADMIN 默认进入 `/admin/dashboard`；REVIEWER 默认进入 `/admin/review`。审核入口在桌面端将双进度轮播放在左侧、积压/待领取/审核中/今日完成四项指标以 2×2 放在右侧，窄屏自动上下排列；轮播每 6 秒切换，箭头、圆点或超过 48px 的拖动会锁定当前页直到离开入口，减少动态效果设置下不自动播放。
