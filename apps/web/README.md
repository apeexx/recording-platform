...
- “任务统计、采集员统计、审核统计”保留三个独立入口，统一支持任务和 Asia/Shanghai 自然日日期范围。采集员与管理员查看审核员时使用可搜索用户选择器；任务统计按工作量、最终结果和参考来源分区展示，并提供五种指标排序的采集员排名，点击排名可带条件进入采集员统计。
- 任务详情为双栏工作台：草稿、运行中和暂停任务均可立即单条添加或导入数据，结束任务拒绝新增。CSV 显式开始后使用稳定任务 ID 每秒刷新真实进度，完成时 Toast 并自动刷新；草稿任务支持安全级联删除。
- 数据池使用服务端分页，编号、四组状态、单个或多个采集员、未分配采集员及结果内容均为 Teleport 高层级悬浮多选菜单，不改变表格尺寸且不受滚动容器裁剪。任务详情支持每页 5/10/20 条并默认 10 条；独立“任务数据池”保持每页 20 条。
...
- 左侧首项为“数据大屏”，使用 `/api/reports/dashboard` 和最近操作接口展示真实任务、条目、采集人数、近 7 日首次提交、任务排行和状态分布，不使用模拟数字。
