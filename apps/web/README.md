...
- 数据池使用服务端分页，并按小程序四组状态、单个或多个采集员、未分配采集员及结果内容筛选。任务详情支持每页 5/10/20 条并默认 10 条；独立“任务数据池”保持每页 20 条。
- 任务详情数据池、独立任务数据池和任务审核池均支持单条、本页全选与“选择全部筛选结果”。跨页预览、快照和执行携带相同筛选条件，避免处理不可见数据；条目“查看”页统一展示参考源、当前结果、条件操作和可继续加载的完整操作记录。
...
- 采集权限页默认分页展示全部小程序用户总数、姓名、用户 ID 和登录账号，支持搜索、直接授权和已授权状态去重。
