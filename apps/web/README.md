...
- 任务详情为双栏工作台：左侧分开放置添加与批量导入卡片，右侧保留宽表格。CSV 可拖入或点击选择，显式开始后通过独立稳定的导入任务 ID 每秒刷新总数/已处理/成功/失败和进度；创建响应使用 `importJobId`，完整状态响应使用 `id`，前端不会再因字段差异中断后续轮询。完成时 Toast 并自动刷新；提供带 BOM 的固定三列示例下载。待领取条目支持弹窗编辑和真正删除，其他状态仍需先释放；草稿任务支持删除。
