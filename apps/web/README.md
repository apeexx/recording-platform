...
- 用户页使用 Web 用户与小程序采集员双 Tab 统一管理：Web Tab 管理 `WEB-...` 后台用户，小程序 Tab 管理 `MINI-...` 采集员。列表与搜索响应使用 `id`、`userType`、`loginName`、name、role、status；支持按姓名、完整前缀用户 ID 或登录名搜索。管理员可在采集员 Tab 修改登录账号、重置采集员密码，也可重置 ACTIVE Web 用户密码；Web 重置后强制下次改密，小程序重置仅废止其活动会话。
