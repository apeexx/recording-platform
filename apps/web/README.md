...
当前不使用 JWT 或 Pinia。通用下拉框、胶囊开关和 1–600 秒双端时长滑块均为无依赖组件；时长组件隐藏原生轨道，使用细灰轨、蓝色选区和白色小圆点，数值随圆点移动，并以圆点半径内缩的单条自定义轨道保证填充、轨道和圆点共用水平中心线。表单校验、操作、搜索、刷新及接口失败统一使用右上角 Toast；只有核心数据首次加载失败使用 `AsyncState` 重试状态。
