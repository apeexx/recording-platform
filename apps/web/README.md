...
- 审核池同时展示 `SUBMITTED`（已提交）与 `REVIEW_PENDING`（待审核），支持编号、状态、采集员、审核员和结果五维表头多选筛选，并将同一筛选快照用于跨页批处理。审核工作台左侧吸顶展示参考源，右侧依次展示只读原始采集结果、AI 候选、独立最终答案和审核结论；视频使用固定画布与 `object-fit: contain`。
- ADMIN 可从审核池进入任务级 AI 设置，分别配置 Qwen 音频转写和文本修订的模型、Prompt、temperature、topP、maxTokens、timeoutMs；密钥始终由服务端读取。AI 音频转写单条录音上限为 20MB；作业刷新后可恢复轮询，完成后必须手动“采用结果”，不会自动保存或自动审核。
...
- 工作统计收敛为管理员可见的“采集员统计”，旧 `/admin/reports/tasks` 重定向至数据大屏，审核统计入口、页面和 API 已移除。采集员统计必须选择任务，默认当天，并提供今天、昨天、近 7 日、本月、全部和自定义范围；任务选择和日期变化会立即查询，不再使用额外查询按钮。任务选择使用编号/名称一体搜索框，自定义日期使用单一范围日历，桌面端位于快捷日期右侧，窄屏完整换行。任务总量与人员排行并列展示按首次提交和首次完成归属的两阶段条数及三类时长，并显示 24 小时首次提交分布、首次/最近提交和高峰时段；十个可排序表头支持全量升降序后再分页。点击人员进入独立详情页，查询条件、排序方向和页码通过 URL 恢复；详情提供双阶段每日统计以及相互独立的提交、完成分页明细。
- 任务详情为双栏工作台：草稿、运行中和暂停任务均可立即单条添加或导入数据，结束任务拒绝新增。CSV 显式开始后使用稳定任务 ID 每秒刷新真实进度，完成时 Toast 并自动刷新；草稿任务支持安全级联删除。
- 数据池使用服务端分页，编号、脚本来源 ID 前缀、四组状态、单个或多个采集员、未分配采集员及结果内容均为 Teleport 高层级悬浮筛选菜单，不改变表格尺寸且不受滚动容器裁剪；来源列显示 sourcePlatform 和可复制 sourceItemId，无绑定时显示“平台内创建”。任务详情支持每页 5/10/20 条并默认 10 条；独立“任务数据池”保持每页 20 条。两个数据池和采集员统计都可导出继承当前筛选的全量 UTF-8 BOM CSV：表头全部为中文，包含首次完成时间，布尔值使用“是/否”；统计日期范围按首次提交或首次完成任一阶段命中取并集，保证两阶段都可对表；先经统一请求链预检会话、权限、任务和日期，再由浏览器原生下载，不受页面分页限制，也不在前端内存中先聚合完整文件。
...
- 采集权限页使用等宽双栏，小程序用户默认每页 10 条，待审批申请和已授权用户仍默认每页 5 条；三者分别维护搜索、页码与每页条数，并展示姓名、用户 ID 和登录账号。
