...
- 审核池同时展示 `SUBMITTED`（已提交）与 `REVIEW_PENDING`（待审核），并将采集员、审核员的用户 ID 与姓名拆列显示。审核工作台使用左右两栏：左侧固定展示参考文本、参考音频、参考视频，右侧统一展示采集结果和审核结论；媒体 URL 优先、历史媒体 ID 回退，缺失项显示“无”。
...
- 任务详情数据池、独立任务数据池和任务审核池均支持单条、本页全选与“选择全部筛选结果”。两个任务数据池的行内操作只保留“查看”，状态调整、释放、废弃和恢复统一收进“调整状态”菜单；跨页预览、快照和执行携带相同筛选条件，避免处理不可见数据。
...
- 采集权限页使用等宽双栏，小程序用户、待审批申请、已授权用户分别维护搜索、页码与每页条数，默认 5 条且可切换 10 条；三者均展示姓名、用户 ID 和登录账号，支持直接授权、审批、撤销和已授权状态去重。
