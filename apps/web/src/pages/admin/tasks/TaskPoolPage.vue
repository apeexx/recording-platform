...
async function load(){if(!taskId.value)return;loading.value=true;error.value='';try{const result=await taskApi.items(taskId.value,page.value,20);rows.value=result.items||[];total.value=result.total||0}catch(e){if(rows.value.length)notifications.error(e.message);else error.value=e.message}finally{loading.value=false}}
