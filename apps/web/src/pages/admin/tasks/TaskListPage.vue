...
const notifications = useNotifications(), rows = ref([]), loading = ref(false), error = ref(''), page = ref(0), size = ref(20), total = ref(0)
...
async function load() { loading.value = true; error.value = ''; try { const result = await taskApi.list(page.value, size.value); rows.value = result.items || []; total.value = result.total || 0 } catch (exception) { if (rows.value.length) notifications.error(exception.message); else error.value = exception.message } finally { loading.value = false } }
function changePage(value) { page.value = value; load() }
function changeSize(value) { size.value = value; page.value = 0; load() }
