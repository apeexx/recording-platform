  ...
  <section class="admin-page review-entry-page">
    ...
    <AsyncState :loading="loading" :error="error" :empty="!rows.length" empty-text="当前没有待审核任务" @retry="load">
      ...
      <section class="review-overview-grid">
        <article><span>当前积压</span><strong>{{ totals.pendingCount }}</strong></article>
        <article><span>待领取</span><strong>{{ totals.submittedCount }}</strong></article>
        <article><span>审核中</span><strong>{{ totals.reviewPendingCount }}</strong></article>
        <article><span>今日完成</span><strong>{{ totals.todayCompletedCount }}</strong></article>
      </section>
...
<style scoped>
.review-entry-page{display:grid;gap:18px}.review-progress-carousel{position:relative;display:grid;grid-template-columns:44px 1fr 44px;align-items:center;gap:16px;overflow:hidden;background:linear-gradient(135deg,color-mix(in srgb,var(--primary) 13%,var(--card)),var(--card))}.review-progress-carousel>button{width:40px;height:40px;border:1px solid var(--border);border-radius:50%;background:var(--card);color:var(--foreground);font-size:24px}.review-progress-slide{display:grid;gap:8px;text-align:center}.review-progress-slide>span{color:var(--muted-foreground)}.review-progress-slide>strong{font-size:42px;color:var(--primary)}.review-progress-slide p{margin:0;color:var(--muted-foreground)}.review-progress-slide>div{height:10px;overflow:hidden;border-radius:999px;background:var(--accent)}.review-progress-slide i{display:block;height:100%;border-radius:inherit;background:var(--primary);transition:width .3s ease}.review-carousel-dots{position:absolute;right:18px;bottom:14px;display:flex;gap:7px}.review-carousel-dots button{width:8px;height:8px;padding:0;border:0;border-radius:50%;background:var(--border)}.review-carousel-dots button.is-active{width:22px;border-radius:999px;background:var(--primary)}.review-overview-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.review-overview-grid article{display:grid;gap:8px;padding:18px;border:1px solid var(--border);border-radius:var(--radius);background:var(--card)}.review-overview-grid span{color:var(--muted-foreground)}.review-overview-grid strong{font-size:30px}.review-task-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}.review-task-card{display:grid;gap:12px;text-decoration:none;color:inherit}.review-task-card>div{display:flex;justify-content:space-between;gap:12px}.review-task-card>div span,.review-task-card small,.review-task-card p{color:var(--muted-foreground)}.review-task-card h3,.review-task-card p{margin:0}.review-task-card progress{width:100%;accent-color:var(--primary)}.review-task-card>strong{color:var(--primary)}@media(prefers-reduced-motion:reduce){.review-progress-slide i{transition:none}}@media(max-width:1000px){.review-task-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:720px){.review-overview-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.review-task-grid{grid-template-columns:1fr}.review-progress-carousel{grid-template-columns:36px 1fr 36px;padding:18px 12px}.review-progress-slide>strong{font-size:34px}}
</style>
