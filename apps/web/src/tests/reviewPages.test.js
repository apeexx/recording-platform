...
describe('审核页面 API', () => {
  ...
  it('按后台角色区分审核池与工作台操作，管理员不会再看到审核员专属按钮', () => {
    ...
    expect(queue).toContain('v-if="isAdmin" class="button-link"')
    expect(workbench).toContain("isReviewer=computed(()=>session.user.value?.role==='REVIEWER')")
    expect(workbench).toContain("canDecide=computed(()=>session.user.value?.role==='ADMIN'||isOwnReviewerAssignment.value)")
