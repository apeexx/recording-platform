...
describe('统一表单控件', () => {
  ...
  it('双端时长滑块复刻已验收原型的胶囊轨道与白色圆点', () => {
    ...
    expect(styles).toContain('background: color-mix(in srgb, var(--primary) 65%, var(--card));')
    expect(styles).toContain('background: color-mix(in srgb, var(--accent) 72%, var(--card));')
    expect(styles).toContain('border: 1.5px solid color-mix(in srgb, var(--primary) 22%, var(--border));')
    expect(styles).not.toMatch(/\.duration-range-selection\s*\{[^}]*background:\s*var\(--primary\)/)
