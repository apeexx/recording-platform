...
describe('统计与操作记录 API', () => {
  ...
  it('统计列表和独立详情展示双阶段、小时分布与两套明细', () => {
    ...
    expect(detail).toContain('DateRangePicker')
    expect(detail).toContain('applyPreset')
    expect(detail).toContain('changeDateRange')
    expect(detail).toContain('requestSequence')
    expect(detail).toContain('今天')
    expect(detail).toContain('近 7 日')
    ...
    expect(detail).toContain("from '../../../lib/businessDate.js'")
  })
})
