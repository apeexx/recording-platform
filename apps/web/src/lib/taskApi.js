...
export const taskApi = {
  ...
  deleteItem(itemId, expectedRevision, key) { return httpRequest(`/api/task-items/${encoded(itemId)}${queryString({ expectedRevision })}`, { method: 'DELETE', idempotencyKey: key }) },
  ...
  grants(id, page = 0, size = 20, filters = {}) { return httpRequest(`/api/tasks/${encoded(id)}/grants${queryString({ page, size, ...filters })}`) },
  ...
  accessRequests(id, page = 0, size = 20, filters = {}) { return httpRequest(`/api/tasks/${encoded(id)}/access-requests${queryString({ page, size, ...filters })}`) },
