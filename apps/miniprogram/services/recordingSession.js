...
function createRecordingSession({ recorder, fs, userDataPath, configuration, onLevel=()=>{}, onDuration=()=>{}, onState=()=>{}, onComplete=()=>{}, onError=()=>{} }) {
  const sampleRate=Number((configuration.sampleRates||[16000])[0]),format=String(configuration.recordingFormat||'WAV').toUpperCase()
  ...
  function fail(error){
    ...
    const failedWriter=writer;writer=null;sampleCount=0;onLevel(0);onDuration(0);stateTo('error');onError(failure);rejectStop?.(failure)
    Promise.resolve(failedWriter?.abort()).catch(()=>{})
    try{recorder.stop()}catch{}
  }
  recorder.onFrameRecorded(({frameBuffer})=>{if(state!=='recording'||!writer)return;sampleCount+=frameBuffer.byteLength/2;onDuration(durationMillis(sampleCount,sampleRate));onLevel(pcmRms(frameBuffer));Promise.resolve(writer.appendPcm(frameBuffer)).catch(fail)})
  ...
  function start(){sampleCount=0;onDuration(0);failure=null;path=`${userDataPath}/recording-${Date.now()}.${format.toLowerCase()}`;const lame=format==='MP3'?require('../vendor/lamejs.iife.js'):null;writer=createAudioFileWriter({fs,path,format,sampleRate,channels:1,bitRate:48,lame});stopPromise=new Promise((resolve,reject)=>{resolveStop=resolve;rejectStop=reject});stopPromise.catch(()=>{});stateTo('recording');recorder.start({duration:Math.min(Number(configuration.maxDurationMillis||600000),600000),sampleRate,numberOfChannels:1,encodeBitRate:48000,format:'PCM',frameSize:4})}
