...
function promptForProfile(){
  return new Promise(resolve=>wx.showModal({title:'请先完善个人资料',content:'设置姓名后，即可申请权限或处理录音任务。数字登录账号可稍后设置。',confirmText:'去设置',success:result=>{if(result.confirm)wx.switchTab({url:'/pages/profile/index'});resolve(false)}}))
}
