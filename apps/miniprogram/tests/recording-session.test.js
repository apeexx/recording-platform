...
test('录音管理器自动停止时也会完成文件并按 PCM 样本计算时长', async () => {
  ...
  const session = createRecordingSession({
    ...
    fs,
    userDataPath: '/tmp',
    version: { sampleRates: [16000], recordingFormat: 'WAV', maxDurationMillis: 1000 },
    onComplete: complete,
  })
  ...
  assert.equal(result.durationMillis, 10)
...
test('流式写入失败会中止临时文件、通知页面并允许重新录制', async () => {
  ...
  const session = createRecordingSession({
    ...
    version: { sampleRates: [16000], recordingFormat: 'WAV' },
