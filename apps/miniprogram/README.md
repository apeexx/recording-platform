...
登录页默认微信快捷登录，可切换 6–12 位非零开头数字账号登录。微信首次进入不再要求填写姓名；资料不完整仍可浏览任务，但申请权限、查看待办、领取和继续作业会提示并跳转 `pages/profile-settings/index`。头像可通过 `chooseAvatar` 选择微信头像并上传，也可恢复本地 `assets/icons/default-collector-avatar.svg` 耳机头像。
