...
本目录是原生微信小程序，面向 `COLLECTOR` 采集员，已实现微信登录、姓名设置、任务权限申请、普通待办/返修列表、参考源展示、录音、文本补充、释放、自动下一条和个人统计。所有任务都必须录音；最终成果为文本时提交录音和文本，最终成果为音频时只提交录音。返修列表展示驳回原因；释放入口位于页面顶端红色垃圾桶并要求二次确认。
