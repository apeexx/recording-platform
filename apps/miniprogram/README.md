...
本目录是原生微信小程序，面向 `COLLECTOR` 采集员，已实现微信快捷登录、数字账号密码登录、独立个人资料与头像、任务权限申请、普通待办/返修列表、参考源展示、录音、文本补充、释放、自动下一条和个人统计。小程序身份 ID 固定为 `MINI-...`，角色由服务端鉴权固定为 COLLECTOR，不作为小程序用户文档字段保存。所有任务都必须录音；最终成果为文本时提交录音和文本，最终成果为音频时只提交录音。
...
“我的”和资料设置页面均展示当前采集员完整的 `MINI-...` userId。点击用户 ID 会复制完整值并提示“用户 ID 已复制”，剪贴板失败或 ID 缺失时提示“复制失败，请重试”；“我的”页使用独立点击事件，复制时不会同时进入资料设置页。个人页同时展示最近 20 条提交记录及其状态。
