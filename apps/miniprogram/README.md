...
任务大厅不展示全局“当前有未完成作业”卡片，也不缓存全局当前条目。每个任务通过“查看任务”进入自己的“任务数据”页；点击“领取新数据”时，同一任务已有普通待录制作业则直接继续该条目，其他任务可分别领取。三个页签固定为：`PENDING`（待录制和待返修）、`SUBMITTED`（可播放、重新录制并保存修改）、`FINISHED`（待审核和已完成，只读播放和查看文本）。每个页签在自身状态集合内统一按最新修改时间倒序、条目序号升序，不再按状态分组；服务端每页固定读取 10 条。提交修改若与审核领取并发并收到 `STALE_STATE`，页面刷新最新状态并提示状态已变化；待审核和已完成隐藏录制、文本编辑、提交及释放操作。
