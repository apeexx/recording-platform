...
任务大厅使用“查看任务”进入“任务数据”页。三个页签固定为：`PENDING`（待录制和待返修）、`SUBMITTED`（可播放、重新录制并保存修改）、`FINISHED`（待审核和已完成，只读播放和查看文本）。每个页签在自身状态集合内统一按最新修改时间倒序、条目序号升序，不再按状态分组；服务端每页固定读取 20 条。提交修改若与审核领取并发并收到 `STALE_STATE`，页面刷新最新状态并提示状态已变化；待审核和已完成隐藏录制、文本编辑、提交及释放操作。
...
下拉刷新只在数据浏览页开启：任务大厅、任务数据和我的统计。任务数据页使用“上一页 / 第 X 页，共 Y 页 / 下一页”显式分页；切换“待处理 / 已提交 / 已完成”页签时回到第一页，下拉刷新保持当前页，数据减少导致页码越界时自动回到新的最后一页，请求失败时保留当前列表。登录、资料设置和录音作业页不得开启下拉刷新，避免打断输入或录音。
