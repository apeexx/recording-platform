...
录音、参考音频、参考视频和录制结果完全独立，可同时运行并产生预期的声音混叠；开始或继续任一媒体不再暂停其他媒体。离开作业页后，音频组件、原生视频和录音会话仍分别按生命周期停止并释放资源。录音暂停继续保持平线且不写入 PCM。录音区外层最小高度为 `460rpx`、状态层为 `392rpx`，作业页专用文字输入框固定为 `200rpx`，不影响其他页面。登录、领取、提交、录音保存和播放等操作错误使用悬浮 Toast，不再在卡片底部追加错误文字；只有页面整体数据加载失败保留带“重新加载”的阻塞状态。
