...
登录页为“砚数声采”A 版，使用正式品牌图标；默认微信快捷登录，可切换 6–12 位非零开头数字账号登录。微信首次进入不再要求填写姓名；资料不完整仍可浏览任务，但申请权限、查看待办、领取和继续作业会提示并跳转 `pages/profile-settings/index`。头像卡本身是原生 `button open-type="chooseAvatar"`，点击后由微信直接提供头像、相册或拍照等来源，选择完成后先进入应用内预览，确认后才上传。卡片下方提供独立的“恢复默认头像”入口，确认后恢复本地 `assets/icons/default-collector-avatar.svg` 耳机头像。上传文件仍由现有后端校验为 JPEG/PNG/WebP 且最大 5MB；头像文件不存在时静默回退默认头像，资料成功刷新会清除旧错误提示。
