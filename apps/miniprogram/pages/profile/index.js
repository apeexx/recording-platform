...
Page({
  ...
  async save(){
    ...
    const wasComplete=this.data.profile.profileComplete
    try{
      let savedProfile
      if(this.data.profile.account){
        savedProfile=await getApp().globalData.session.setName(name)
        ...
        savedProfile=await getApp().globalData.session.completeProfile(wantsAccount?{name,account,password}:{name})
        this.applyProfile(savedProfile)
        feedback.success(wasComplete?'资料已更新':'资料已完成')
      }
      await this.load()
