...
- commit ID：待提交后补记
- 修改内容：
  - 条目创建在幂等重放、任务状态、来源绑定和参考内容校验通过后，读取任务内已有条目的最大 `sequence`。
  - MongoDB 以单次 `findAndModify` 原子分配“任务持久化 `itemSequence` 与当前最大 `sequence` 中较大值加一”；历史计数器落后时自动抬高，计数器已高或保存失败造成的间隔不回退、不复用。
  - 管理员新增、CSV 导入与外部集成继续共用同一创建服务，未改变外部请求参数、响应结构或机器接口。
  - 新增最大序号读取、历史 83 条接续为 84、高计数器不回退、幂等重放/重复来源不消耗编号及原子更新结构回归测试。
- 验证结果：
  - 定向 Maven 测试 `TaskItemCreationServiceTests,MongoTaskItemStoreTests,MongoTaskStoreTests` 35/35 通过。
  - 全量 `backend\mvnw.cmd test` 已执行，368 项中 329 项通过、0 个断言失败；其余 39 个错误来自 5 个依赖完整 Spring 上下文的测试类，均因本机 `localhost:27017` 连接被拒绝而无法加载上下文。未启动 MongoDB、未修改数据库，也不将全量测试记为通过。
