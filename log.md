...
- 针对微信审核反馈，在小程序登录首页的所有登录操作之前增加常驻“内部使用说明”，明确仅面向重庆砚数科技有限公司内部员工及经授权的录音采集人员，并说明录音任务用途和未受邀人员边界。
- 微信快捷登录与数字账号登录说明同步改为内部及授权人员表述；不增加游客模式或阻断弹窗，不修改登录、会话接管、后端 API、权限或 MongoDB。
- 按 TDD 增加首页说明、DOM 顺序、固定文案、样式和双登录入口回归契约；红灯确认旧页面缺少审核说明，最小实现后专项测试 11/11 通过。
