...
- 审核工作台在“释放审核”旁增加“标记为无效”；当前领取审核员或管理员确认后可将 `REVIEW_PENDING` 条目原子转为 `DISCARDED`，固定记录无效原因并保留采集结果、媒体、采集与审核归属，管理员恢复后回到原审核状态。
- 新增 `POST /api/reviews/{itemId}/discard`，沿用 Web CSRF、持久化幂等和 revision/CAS；操作历史新增 `REVIEW_DISCARD`，并纳入现有废弃次数统计，不扩大通用任务条目废弃接口的审核员权限。
- 新增 Web 路由状态、缓存、审核 API 与工作台交互测试，以及后端服务、控制器、Mongo 原子更新和统计测试；不新增依赖、Mongo 字段或数据迁移。

## 2026-08-03 完善 Web 端用户操作手册截图

- 使用本机 MongoDB 和全虚拟本地夹具构造 3 个演示任务、3 名演示采集员以及授权、待审批、提交、审核、返修、完成和废弃状态；夹具按 `DEMO-MANUAL-20260803` 批次记录，可根据本地忽略清单选择性清理，未导入真实业务数据。
- 为 Web 手册补充首次登录、数据大屏、任务列表与配置、任务数据池、采集权限、审核入口/审核池/工作台、采集员统计、语音生成、用户、邀请码、操作记录和个人账号共 15 张标注截图。
- 所有入库截图仅包含虚拟姓名、账号、任务和结果；邀请码成功弹层已替换为 `XXXX-XXXX-XXXX` 示意码，原始完整本地邀请码和未标注截图未进入仓库。
- 语音生成截图只展示现有工作台，不调用真实生成服务；本次未修改业务代码、API、MongoDB 结构、权限规则或项目依赖。

## 2026-08-02 增加双端用户操作手册

- 在仓库根目录新增 Web 端与小程序端中文用户操作手册，分别覆盖 ADMIN/REVIEWER 后台流程和 COLLECTOR 小程序采集流程，并在 README 增加直接入口。
- Web 手册按角色说明登录、任务、数据池、权限、审核、统计、语音生成、用户、邀请码和操作记录；小程序手册说明首次邀请准入、资料、授权、领取、录音、提交、修改、释放、废弃、恢复、返修和双阶段统计。
- 新增线上登录页脱敏红框截图；登录后 Web 页面和小程序开发者工具/真机画面因当前缺少可安全复用的已登录截图，改为提供逐项补图文件名、页面状态、构图与标注清单，不伪造业务画面。
- 本次仅修改文档和用户手册图片，不修改业务代码、API、数据库、权限或项目依赖。

## 2026-08-02 增加释放冷却并统一 Web 品牌图标

- 采集员本人释放条目后 30 分钟内，原子领取查询按既有 `RELEASE` 操作历史跳过同一条；其他采集员仍可立即领取，管理员释放不触发原采集员冷却，恰好满 30 分钟恢复领取资格。
- 保持领取与释放 API、幂等、revision/CAS、序号排序和 `NO_AVAILABLE_ITEM` 错误契约不变；未新增 Mongo 字段、集合、索引、迁移或项目依赖。
- Web favicon、登录页和管理端统一复用“砚数声采”品牌 SVG，页面标题改为“录音任务平台”，删除不再使用的 Vite 默认 favicon；后台内部菜单图标和首次改密页保持不变。
- 新增服务、Mongo 查询和 Web 静态回归测试，覆盖本人冷却、他人可领、30 分钟边界、池空、管理员释放及品牌资源引用。

## 2026-07-31 修正审核进度并限制小程序姓名重复

- 审核处理进度改为只统计当前处于已提交、审核中、返修中或已完成的条目，已处理数仍为返修中与已完成；待录、可领取和废弃不再进入分母。
- 审核任务卡首行同行展示任务编号、名称和积压数，下一行并排展示任务完成度与审核处理进度；任务详情数据池恢复默认每页 10 条，独立数据池继续默认 20 条。
- 小程序姓名增加唯一稀疏索引，完善资料和修改姓名均在去除首尾空格后精确检查重名，并将并发冲突统一返回 `409 NAME_EXISTS`；英文大小写不同允许共存，停用用户继续占用原姓名。
- 未新增依赖或迁移脚本；部署前需确认现有 `miniprogram_users` 不存在重复非空姓名。

## 2026-07-30 修复审核任务汇总接口异常

- 修复 `/api/reviews/tasks` 和单任务审核汇总在构造 Mongo 聚合条件时，使用不允许 `null` 的 `List.of` 导致 `NullPointerException` 并返回 `INTERNAL_ERROR` 的问题；改用可表达 Mongo 空值比较的列表结构，审核指标口径和响应契约保持不变。
- 新增不依赖外部 Mongo 服务的聚合管道回归测试，验证缺少首次提交时间时仍能正确构造空值比较条件，避免部署构建环境因本地数据库不可用而失败。

## 2026-07-30 优化统计交互、统一分页与审核界面

- 采集员统计日期范围在确定开始日后支持悬停预览，十个表头改为降序、升序、默认三态循环；默认排序不写 URL，自动刷新保留旧数据并显示圆形加载层，列表与详情把页码和每页条数写入查询参数。
- Web 后台数据列表统一为数字分页器，用户、邀请码、操作记录和语音生成记录改为真实服务端分页；普通列表默认 20 条，操作记录默认 50 条，采集权限继续使用 5/10。采集权限三块列表预留满页高度并将分页固定在卡片底部。
- 审核任务摘要新增有效条目、完成、审核进入、审核处理、待领取、审核中和今日完成指标；列表与单任务摘要使用一次 Mongo 聚合，列表只返回有积压任务并稳定排序。
- 审核入口增加任务整体进度/审核处理进度轮播、四项常驻指标和任务运营卡；手动拖动、箭头或圆点操作后锁定当前页，减少动态效果时禁用自动轮播。审核池改为任务指标、独立五维筛选、选中后批量操作、异步进度和统一分页布局。
- “录音审核”和“工作统计”改为侧边栏直接入口并保持子路由高亮；小程序删除录音按钮移除图标，仅保留文字，原删除语义不变。未新增依赖、Mongo 字段或迁移。

## 2026-07-30 增加采集员双阶段统计与中文 CSV

- `task_items` 新增可空 `firstCompletedAt`：首次进入完成状态时写入，管理员退回、废弃、恢复及再次完成均保留，真正释放回池时与首次提交锚点一起清除；启动时从可靠的完成操作历史幂等回填缺失值，并记录无法判断的脱敏数量。
- 管理员采集员统计拆为“提交统计”和“完成统计”：日期分别按 Asia/Shanghai 的 `firstSubmittedAt` 与 `firstCompletedAt` 独立筛选，两阶段均提供条数、最终录音、参考音频、参考视频时长；新增固定 0–23 点首次提交分布、两阶段每日统计、排行时间摘要和按首次完成时间分页的完成明细。
- Web 人员查看由同页下钻改为独立详情路由；列表查询的任务、日期、排序和页码写入 URL，返回时可恢复。详情页提供双阶段汇总、个人小时分布、双阶段每日表和相互独立的提交/完成分页标签；刷新失败继续保留已显示数据。
- 全量 CSV 固定表头全部改为中文，新增“首次完成时间”，是否包含结果录音输出“是/否”；统计日期按首次提交或首次完成任一命中取并集，确保两阶段均可对表；继续保留 BOM、稳定全量输出、标准转义、公式注入保护、URL 参数脱敏和仅完成状态输出最终文本的规则。
- 小程序 `/api/reports/me/**` 契约与统计页面保持不变；未新增依赖，不执行破坏性数据迁移。

## 2026-07-30 重做工作统计与数据池 CSV 导出

- 工作统计收敛为管理员数据大屏和采集员统计：旧任务统计地址重定向至大屏，审核统计前端页面、导航、接口、DTO、聚合和安全规则全部移除；大屏新增服务端生成时间与保留旧数据的刷新状态。
- 采集员统计改为任务必选、当天默认和六种日期范围，提供五项任务汇总、五指标排序的采集员分页总表，以及同页人员汇总、每日提交/当前完成/三类时长、最近提交和分页完整明细。
- 两个 Web 数据池新增脚本来源列、可复制 sourceItemId 和按字面前缀匹配的来源筛选；跨页批处理同步继承来源条件。
- 新增管理员全量 CSV 导出：继承数据池和统计日期筛选，按条目序号稳定分页流式写出 UTF-8 BOM CSV，支持空结果、标准转义和公式注入保护；最终文本仅对已完成条目输出，参考音视频 URL 移除查询参数与片段，不包含结果录音地址、完整签名 URL、磁盘路径、技术参数或凭证；Web 先经统一请求链预检会话和错误，再使用浏览器原生下载，避免在前端内存聚合完整文件。
- 不新增依赖、不修改 MongoDB 字段、不执行数据迁移；统计继续按 Asia/Shanghai 的 firstSubmittedAt 和当前 assignment 口径计算。

## 2026-07-29 修复审核筛选候选请求缺少任务 ID

- 修复审核池采集员、审核员筛选组件未传入当前任务 ID，导致页面挂载时请求 `/api/reviews/tasks//filter-users` 并返回 400 的问题。
- 新增真实路由与审核池组件挂载回归测试，验证两类候选请求都携带当前 `taskId`，且不会再生成双斜杠路径。
- 不修改后端接口、权限、筛选语义或数据库结构，不增加依赖。

## 2026-07-28 优化任务筛选、条目详情与采集权限

- 任务详情与独立数据池将编号、状态、采集员和结果统一为 Teleport 固定定位的悬浮多选菜单；编号支持搜索候选后跨页勾选，状态与结果支持同维度多选，人员继续支持单选或多选及未分配。
- 两个数据池的行内操作统一只保留“查看”，原有状态变更、释放、废弃和恢复合并到“调整状态”菜单；跨页批处理同时保存编号、状态、人员、未分配和结果条件，并兼容旧单值筛选请求。
- 条目详情改为左侧参考源原位编辑，右侧依次展示条目信息、采集结果和最近 3 条操作记录；完整记录按最大 100 条分批读取，在支持遮罩、Esc、重试与独立滚动的弹窗中展示。
- 采集权限改为等宽双栏，小程序用户、待审批申请和已授权用户各自支持姓名/完整用户 ID/登录账号搜索、数字分页及 5/10 条切换；权限查询关联小程序用户后统计分页，并返回登录账号。
- 不增加前端或后端依赖，不新增数据库字段或迁移；保留已有 revision/CAS、幂等、媒体、释放与废弃语义。

## 2026-07-27 完善参考源、采集员废弃与跨页批处理

- Web 审核工作台固定展示参考文本、参考音频和参考视频，远程 URL 优先、历史媒体 ID 回退，缺失项明确显示“无”。
- 小程序任务数据新增独立“废弃数据”页签；采集员可对本人待录制或待返工数据填写原因并标记无效，在只读详情查看原因、操作人和时间后恢复。释放操作新增可选备注，顶部图标区分三角回收与垃圾桶。
- 后端扩展废弃、恢复、释放备注和 `currentDiscard` 当前标记；所有流转保留幂等、revision/CAS、操作历史和媒体读取边界，采集员废弃次数同步纳入流程统计。
- 任务详情数据池、独立任务数据池和审核池新增单条、本页、跨页三级选择。跨页操作使用 `batch_operation_jobs`、`batch_operation_items` 固化快照，按条执行并保存租约、检查点、成功、失败和状态变化跳过结果，服务重启后可续跑。
- 新增后端批处理与废弃权限测试、Web 选择/API/媒体回退测试、小程序废弃与恢复测试；不增加依赖，不执行数据库迁移。

## 2026-07-27 完善统计筛选、采集员排名与审核池批量操作

- 小程序任务统计增加单日选择和清除日期，默认仍展示全部日期；切换任务、日期或重新进入页面时查询当前数据库，“更多提交”继承当前任务和日期条件。
- Web 的任务、采集员和审核统计增加任务及 Asia/Shanghai 自然日日期范围筛选；采集员和管理员选择审核员改为姓名、用户 ID 或登录账号搜索。任务统计按工作量、最终结果、参考来源分区展示五项当前指标，并增加五种指标排序的采集员排名及带筛选条件跳转。
- 统计接口保留旧汇总字段并增加明确的提交、完成、最终录音、参考音频和参考视频字段；当前统计排除已释放回数据池及已废弃条目，任务采集员排名使用分页响应。
- 任务数据池和审核池将采集员、审核员的用户 ID 与姓名拆列显示，姓名由身份目录批量补全，不写入任务条目。
- 审核池支持混合勾选；批量领取和批量分配只处理所选“已提交”，批量通过只处理所选“待审核”。所选条目批量领取/分配接口保持外层及逐条幂等、revision/CAS，并返回逐条结果供页面汇总提示。
- 不增加依赖、不执行数据库迁移；后端 Maven 全量测试 326/326、Web Node 测试 12/12 与 Vitest 66/66、小程序 Node 测试 110/110 通过，Web 生产构建、修改 JavaScript 语法、JSON/页面路径和 `git diff --check` 均通过。

## 2026-07-27 修复小程序提交、参考媒体加载与任务统计

- 纯文本作业改为通过同一提交路径发送标准 JSON；包含录音时继续使用 multipart，并复用同一后端提交、幂等、状态与参考媒体时长校验逻辑，移除小程序手工拼接 multipart 导致的偶发 `INVALID_REQUEST`。
- 参考音频在 `canplay` 尚未提供正时长时自动进行有界延迟检测，取得时长后立即回传作业页；换源和组件销毁会取消旧检测，不再要求用户先点击播放。
- 修复任务统计将 `task_items.taskId` 字符串直接与 MongoDB `tasks._id` ObjectId 关联导致 `items` 为空的问题；任务列表和指定任务统计现在按实际存储 ID 读取任务资料，统计口径不变。
- 增加小程序 JSON 提交与音频延迟时长测试，以及后端 JSON 提交和 ObjectId 统计关联测试；同步更新根 README、小程序 README 与接口说明。
- 验证结果：小程序全量 Node 测试 108/108、后端全量 Maven 测试 320/320 通过。

## 2026-07-27 合并小程序“我的”与个人资料设置

- “我的”Tab 直接展示头像、基本信息、数字账号、密码修改和退出登录，页面标题统一为“个人资料设置”，不再经过资料摘要和第二层设置页面。
- “基本信息”右侧始终显示资料状态：未完成姓名设置时为橙色“待完善”，完成后为绿色“已完善”；用户 ID 整行复制、头像确认保存、资料校验和改密重新登录行为保持不变。
- 删除独立 `profile-settings` 页面及路由；任务入口发现资料未完善时，确认“去设置”改为直接切换到“我的”Tab。
- 本次不修改后端 API、MongoDB 或身份规则；TDD 页面与守卫针对性测试 32/32、保存状态专项测试 12/12、小程序全量测试 105/105 通过。

## 2026-07-27 增加小程序独立任务统计与提交时长核对

- 小程序底部导航调整为“任务 / 统计 / 我的”，将原个人页统计和提交记录迁入独立统计页；进入时默认选择最近提交任务，展示五项任务总量、按首次提交日期分段的每日数据、最近 3 条提交和倒序完整记录。
- 统计直接聚合 `task_items` 当前 assignment：同一条目只计一次，返修和覆盖不重复增加；录音取最新结果时长，参考音频和视频分别累计；管理员释放回数据池后清除当前统计锚点，历史提交与操作记录仍保留。
- 录音作业参考文本支持长按选择及“复制全文”；每次提交上报参考音视频时长，不存在的媒体固定为 0，存在但未取得正时长时阻止提交。后端保存首次基准，后续误差不超过 1000 毫秒时沿用基准，超过时拒绝。
- 新增采集员任务列表、任务统计及任务提交记录接口，同步更新根 README、小程序 README、接口和 MongoDB 字段说明；按已确认方案不兼容旧统计数据，更新后重置数据库。
- 验证结果：后端全量 Maven 测试 318/318、小程序全量 Node 测试 103/103 通过；针对性后端 clean 测试 38/38 通过。

## 2026-07-27 完善小程序账号使用与鉴权说明

- 针对微信再次审核反馈，将登录板块下方标题改为“账号使用与鉴权说明”，明确小程序不面向公众开放注册，仅限公司内部员工及经项目管理员邀请、授权的录音采集人员使用。
- 首页进一步说明登录时核验微信身份或数字账号、进入录音任务前核验对应任务授权，以及未通过核验的人员不能领取、录制或提交任务。
- 本次只调整首页审核文案及其回归契约，不修改微信/数字账号双登录、会话接管、任务授权、后端 API 或 MongoDB。

## 2026-07-26 重构根 README 项目入口

- 参照标注脚本中心 README 的信息层级，将根 README 调整为项目定位、当前能力、前置环境、快速开始、开发验证、生产部署、目录导航、文档入口与安全边界。
- 根 README 不再重复维护完整 API、MongoDB 集合、模块实现和历史验收明细；相关内容分别链接到 `AGENTS.md`、模块 README、`scripts/README.md` 和本日志。
- 保留现有启动、就绪检查、验证和生产部署命令，不修改业务代码、接口、配置或数据库。

## 2026-07-26 补充小程序内部使用范围说明

- 针对微信审核反馈，在小程序登录板块下方常驻展示“内部使用说明”，明确仅面向重庆砚数科技有限公司内部员工及经授权的录音采集人员，并说明录音任务用途和未受邀人员边界。
- 微信快捷登录与数字账号登录说明同步改为内部及授权人员表述；不增加游客模式或阻断弹窗，不修改登录、会话接管、后端 API、权限或 MongoDB。
- 按 TDD 增加首页说明、DOM 顺序、固定文案、样式和双登录入口回归契约；位置调整先以说明卡位于登录卡之前的旧结构验证红灯，移动至登录板块下方后专项测试 11/11 通过。
...
- commit ID：待提交后补记
- 修改内容：
  - 在既有机器单条写入接口之外增加完成结果查询与当前结果录音读取，继续使用同一 `INTEGRATION_IMPORT` 专用 API Key。
  - 结果查询只在 `COMPLETED` 暴露当前文字和音频可用性；未完成、待审核与返修状态固定隐藏当前结果内容。
  - 录音读取只接受完成条目的当前 `RECORDING` 媒体，复用既有 200/206 单 Range 流与文件存在性校验，不开放新的公开录音路径。
  - API Key 过滤器与 Spring Security 白名单精确扩展到三条集成端点；Cookie、Bearer 和 CSRF 不能替代机器 Key，机器身份仍不能访问其他 API。
  - 未新增依赖、MongoDB 字段或前端调用，未修改公开参考媒体规则。
- 验证结果：
  - TDD RED：定向 Maven 测试因结果服务与视图尚不存在而出现预期测试编译失败。
  - 最小实现后定向测试 25/25 通过；完整后端测试 303/303 通过，`git diff --check` 与窄范围敏感信息扫描通过。
...
## 2026-07-25 增加生产一键部署脚本

- 新增 Linux 生产部署入口，使用干净 `main` 门禁、仅快进 Git 更新和拉取后脚本重载，依次完成 Web、后端与微信小程序的依赖安装、测试和生产构建。
- 新增无密钥 PM2 单实例配置与阿里云 Maven 镜像配置；全部构建通过后才启动或重启后端，并以本机就绪接口的 `overall`、`mongo`、`storage` 全部为 `UP` 作为成功条件。
- 部署脚本不创建或修改 `.env`、MongoDB、Nginx、Certbot、备份及微信发布状态；首管理员完成首次改密并删除初始化凭据后仍可继续部署。
- 新增静态契约测试覆盖 Git 安全门禁、三端执行顺序、国内镜像、PM2 单实例、就绪检查和秘密值边界；脚本测试 18/18、Web Node 12/12 与 Vitest 62/62、Web 生产构建、小程序 97/97、后端 303/303 均通过。
- 已使用 Git Bash 完成 `bash -n` 语法检查；实际服务器脚本执行、PM2 启动和公网 HTTPS 验收仍需服务器拉取本次提交后完成。

## 2026-07-25 修复生产部署 Maven Wrapper 权限

- 将 `backend/mvnw` 的 Git 文件模式修正为 Linux 可执行，避免服务器首次手动执行 Wrapper 后产生仅权限变化的脏工作区，并被生产部署安全门禁拒绝。
- 新增 Git 文件模式回归测试，确保后续提交仍将 Maven Wrapper 保持为 `100755`；未修改 Wrapper 内容、构建逻辑或生产配置。
# 2026-07-28 Web 数据大屏与任务工作区优化

- 将管理员首入口“工作台”替换为真实数据大屏，新增 `/api/reports/dashboard`，展示任务/条目汇总、当前采集人数、Asia/Shanghai 最近 7 日首次提交趋势和任务排行；最近操作继续复用现有接口。
- 任务详情与独立数据池新增小程序四组状态、单/多采集员、未分配采集员和结果内容筛选；跨页预览、快照和执行携带同一筛选条件。
- 新增条目查看页，集中展示参考源、采集结果、人员状态、条件操作与分页操作时间线；旧操作记录路由兼容重定向。
- 采集权限页默认分页展示全部小程序用户并标记授权状态；审核工作台调整为左侧参考源、右侧采集结果与审核结论的两栏结构。

# 2026-07-28 增加小程序邀请码准入与优化登录首页

- 新微信身份首次创建账号前必须使用管理员生成的邀请码；已有小程序用户、数字账号登录和会话接管保持不变，任务授权继续独立校验。
- 新增邀请码哈希存储、微信身份兑换声明和 Mongo 原子名额预留，同一身份重试不重复扣次，并发兑换不突破使用上限；完整邀请码只在创建响应显示一次。
- Web“系统管理”新增 ADMIN 邀请码管理页，支持创建、一次性复制、脱敏分页、剩余次数与状态查看、永久停用；不提供编辑、删除、重新启用或兑换人员明细。
- 小程序登录首页先展示“特定范围使用说明”，账号切换入口移至标题行右侧；首次微信登录按需打开邀请码弹层，提交时重新获取微信临时 code，失败保留输入。
- 同步补充后端接口、MongoDB 集合和三端使用说明；未新增依赖，审核截图与运行视频仍需在提审页面另行上传，不进入仓库。
- 后端 355/355、小程序 118/118、Web Node 13/13 与 Vitest 80/80 测试通过；Web 生产构建、10 个修改 JavaScript 文件语法检查和 `git diff --check` 通过。

# 2026-07-28 调整小程序登录页顺序与邀请码按钮

- 将微信/账号登录卡片移动到品牌区之后，“特定范围使用说明”调整到登录卡片下方，使微信登录主体位于页面中部；说明文案和账号切换位置保持不变。
- 邀请码弹窗操作区改为响应式双列，验证按钮比取消按钮更宽；统一按钮盒模型、行高和内边距，并禁止文字换行，避免加载图标挤压“验证并登录”。
- 未修改登录、邀请码、会话接管、任务授权、后端或数据库逻辑，未新增依赖。
- 小程序全量测试 118/118、修改测试文件语法检查和 `git diff --check` 通过。

# 2026-07-28 记录小程序用户邀请码准入来源

- 新微信身份首次准入时，将邀请码 ID、名称快照、末四位和兑换时间写入 `miniprogram_users`；历史用户不回填且继续兼容。
- 管理员用户摘要增加可空的邀请码来源字段，Web 用户管理来源列展示“后台创建”、脱敏邀请码来源或“历史用户”，不返回完整邀请码、哈希或 OpenID。
- `miniprogram_invitation_codes`、`miniprogram_invitation_claims` 与 `sessions` 继续独立存储；会话保持现有 `expiresAt` TTL 自动清理和接管语义。
- 后端 362/362、小程序 118/118、Web Node 13/13 与 Vitest 85/85 测试通过；Web 生产构建、修改 JavaScript 文件语法检查和 `git diff --check` 通过。

# 2026-07-28 优化审核体验并增加 AI 辅助审核

- 任务详情与独立数据池增加按状态展示的查看、释放、废弃和恢复快捷操作；采集权限页仅将小程序用户默认分页调整为 10 条。
- 审核池增加编号、状态、采集员、审核员和结果五维筛选，并将筛选条件同步到跨页预览、快照和执行；管理员和审核员均可释放本人占用的审核数据。
- 新增独立 `reviewFinalAnswer`，审核通过不再覆盖原始采集文本或录音；外部完成结果优先返回审核最终答案。
- 新增任务级 Qwen 音频/文本 AI 配置、24 小时 TTL 持久化作业、并发 2 的有界队列、租约恢复和 OpenAI-compatible SSE 解析；AI 结果只供人工采用，不自动保存或决定。
- 审核筛选候选改由审核域按当前角色可见条目生成，避免审核员访问管理员接口；AI 音频限制为 20MB，并使用每次领取唯一租约令牌避免过期作业写回覆盖。
- 审核工作台重构为只读原始结果、AI 候选、独立最终答案和审核结论四段式布局，并增加管理员 AI 设置页。

# 2026-07-30 优化统计筛选与录音删除

- 采集员统计将任务搜索和选择合并为单一组合框，选择任务、快捷日期或完整自定义范围后立即刷新；新增无依赖单月范围日历，并移除筛选区查询按钮和排序下拉框。
- 人员排行的提交/完成八项指标及首次、最近提交时间改为表头排序，新增 `sortDirection=asc|desc`，后端对全量数据稳定排序后分页，空时间固定置后。
- 小程序录音结果增加“删除录音”，确认后清空录音但保留文本；本次文件尽力清理，服务器已有录音只在后续保存修改时真正移除。
- 未新增依赖、MongoDB 字段或数据迁移；按测试驱动补充后端、Web 和小程序回归测试。

# 2026-07-30 优化管理端布局与 AI 设置

- 采集权限页解除左右列等高拉伸，小程序用户卡片按自身 10 行列表和分页独立收口，右侧待审批与已授权卡片继续按各自 5 行高度排列。
- 采集员统计任务选择器统一编号与任务名称的单行基线，编号保留固定宽度，超长任务名称使用省略号。
- AI 辅助审核设置保留音频和文本左右双卡，改用现有胶囊开关与自定义下拉框，并增加分区参数卡、范围提示和 Prompt 字符计数。
- 审核入口将双进度轮播与四项审核指标合并到同一概览行，桌面端采用左侧轮播、右侧 2×2 指标布局，窄屏自动上下排列。
- 未修改审核接口、AI 配置字段、数据库或状态流转，未新增依赖。

# 2026-07-31 保留审核入口无积压概览

- 审核任务列表新增兼容参数 `includeCleared`，默认仍只返回有积压任务；启用后返回全部人工审核任务并保留零积压任务的真实进度与今日完成指标。
- Web 审核入口使用完整人工审核任务汇总计算双进度和四项指标，仅将仍有积压的任务渲染为待办卡。
- 所有积压处理完成后继续展示审核概览，并在任务区提示“所有审核积压已处理完成”；未新增依赖、数据库字段或迁移。

# 2026-07-31 增加通用说明并统一凌晨四点业务日

- Web 新增无依赖通用问号说明，支持悬浮、键盘聚焦、点击固定、外部点击和 Esc 关闭；审核入口、AI 辅助审核设置和任务详情参考源字段已接入。
- 审核轮播禁止文字选择，问号交互不会触发拖动；审核任务卡改为普通卡片与独立审核池链接，避免交互元素嵌套。
- 数据大屏、审核今日完成、管理员采集员统计/明细、小程序单日统计和 CSV 日期筛选统一按 Asia/Shanghai 04:00 至次日 04:00 业务日归属，原始时间字段不修改。
- 24 小时首次提交分布继续使用真实北京时间小时，固定按 04、05…23、00…03 返回并展示零值。
- 未新增依赖、MongoDB 字段或数据迁移。

# 2026-07-31 完善小程序双阶段统计与全局说明

- 小程序任务统计默认当前业务日，新增今天、昨天、近 7 日、本月、全部和底部单月范围日历；提交与完成分卡展示条数和三类时长，24 小时柱图按 04→03 压缩铺满并支持点击提示。
- 小程序新增提交/完成每日卡片和两套独立 5 条分页明细；筛选刷新保留旧内容，使用请求序号防止旧响应覆盖最新条件，旧完整提交记录页继续兼容。
- `/api/reports/me/tasks/{taskId}` 保留旧字段与 date 参数，并新增 fromDate/toDate、stageSummary、stageDays；本人提交明细支持范围，新增只返回当前已完成条目的 `/completions` 分页接口及 firstCompletedAt。
- Web 采集员详情复用六种日期选择和范围日历，日期变化同步 URL、重置两套明细页码并让 CSV 继承当前范围。
- 审核大轮播标题使用左右等宽占位恢复严格居中，小任务卡移除重复问号；通用问号修复多实例 ID，并覆盖大屏、任务、审核、统计、系统管理和语音生成关键规则。
- 未新增依赖、MongoDB 字段或迁移。

# 2026-07-31 优化小程序统计明细分页

- 小程序“提交明细 / 完成明细”统一调整为每页 10 条，两个标签继续独立保存当前页。
- 分页中部改为蓝色胶囊原生页码选择器，可直接跳转任意有效页；加载期间禁用分页操作，首页和末页禁用对应方向按钮。
- 任务或日期变化时两套页码共同回到第一页；刷新后总页数减少时自动收敛到新的最后有效页并重新加载。
- 未修改后端接口、Web、数据库或统计口径，未新增依赖。
