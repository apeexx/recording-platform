...
以下画面需要登录后的脱敏测试数据。补图时请避免真实姓名、账号、完整邀请码、媒体地址和客户内容。

| 建议文件名 | 页面或状态 | 需要标注的区域 |
|---|---|---|
| dashboard-overview.png | 管理员数据大屏 | 刷新按钮、业务指标和最近操作 |
| task-create.png | 创建任务 | 基本信息、参考类型、成果类型和录音配置 |
| task-detail-data.png | 任务详情 | 添加数据、CSV 导入、数据池 |
| permissions-overview.png | 采集权限 | 用户、待审批申请、已授权用户 |
| review-task-select.png | 审核入口 | 双进度、四项指标和任务卡 |
| review-queue.png | 任务审核池 | 五维筛选、列表选择和批量操作 |
| review-workbench.png | 审核工作台 | 参考源、原始结果、最终答案和审核结论 |
| collector-statistics.png | 采集员统计 | 任务选择、日期范围和人员列表 |
| voice-workbench.png | 语音生成工作台 | 模式选择、输入区和生成按钮 |
| users-tabs.png | 用户管理 | Web/小程序页签、搜索和创建账号 |
| invitation-created.png | 新邀请码成功弹层 | 一次性完整邀请码和复制按钮，必须使用虚拟码 |
