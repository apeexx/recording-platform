...
| 微信小程序 | `apps/miniprogram/` | 邀请准入、采集员登录、唯一姓名资料、权限申请、任务领取、录音/文字提交、返修、结果查看和按任务统计 |
...
| 领域 | 已实现能力 |
|---|---|
| Web 管理 | ADMIN/REVIEWER 登录与会话接管、角色路由、真实数据大屏、统一数字分页、任务配置、带脚本来源筛选和中文全量 CSV 导出的数据池、按当前审核状态计算处理进度且清空积压后仍保留真实汇总的双进度审核入口、审核池独立五维筛选与跨页批处理、任务级 AI 辅助审核，以及支持自动查询、悬停范围预览、三态排序、24 小时分布和独立人员详情的双阶段采集员统计 |
...
所有“今天”、日期筛选、每日统计和 CSV 日期范围统一按 Asia/Shanghai 凌晨 4 点换日：例如 `2026-07-31` 表示 `2026-07-31 04:00` 至 `2026-08-01 04:00` 的左闭右开区间。原始提交/完成时间及普通操作时间仍按实际时间展示。审核入口、AI 设置和任务详情使用通用问号说明，支持悬浮、键盘聚焦和点击固定。
