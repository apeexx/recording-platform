...
| 微信小程序 | `apps/miniprogram/` | 邀请准入、采集员登录、唯一姓名资料、权限申请、任务领取、录音/文字提交、返修、结果查看和按任务双阶段统计 |
...
| 领域 | 已实现能力 |
|---|---|
| Web 管理 | ADMIN/REVIEWER 登录与会话接管、角色路由、真实数据大屏、统一数字分页、任务配置、可从详情返回恢复筛选和分页且长期记忆所选任务的数据池、已授权采集员的单页/跨页批量分配、中文全量 CSV 导出、固定两位小数的进度百分比、双进度审核入口、审核池独立五维筛选与跨页批处理、审核领取后的标记无效、任务级 AI 辅助审核，以及列表与人员详情均可快捷切换日期的双阶段采集员统计 |
| 小程序采集 | 新微信身份邀请码首次准入、已有微信身份与数字账号登录、“我的”直达资料与头像设置、任务授权、领取、录音、当前录音删除、文本复制/补充、纯文本 JSON 与录音 multipart 提交、参考媒体时长自动检测、返修、释放备注、本人释放后 30 分钟内跳过同一条、标记无效与恢复，以及包含六种日期范围、双阶段汇总、小时分布、每日统计和两套分页明细的任务统计 |

所有“今天”、日期筛选、每日统计和 CSV 日期范围统一按 Asia/Shanghai 凌晨 4 点换日：例如 `2026-07-31` 表示 `2026-07-31 04:00` 至 `2026-08-01 04:00` 的左闭右开区间。原始提交/完成时间及普通操作时间仍按实际时间展示。Web 大屏、任务、审核、统计、系统管理和语音生成的关键复杂规则使用通用问号说明，支持悬浮、键盘聚焦、点击固定和单实例切换。
| 任务与审核 | 草稿、运行、暂停、结束状态；固定展示文本/音频/视频三类参考源；文字或录音成果；运行中任务的待领取数据可批量分配给已授权采集员；人工审核、驳回返修、审核标记无效与软恢复、单页/跨页批量操作和状态并发控制；AI 音频转写单条上限 20MB |
...
小程序姓名去除首尾空格后按精确值全局唯一，停用用户继续占用原姓名；英文大小写不同的姓名允许共存。完善资料或修改姓名发生冲突时返回 `409 NAME_EXISTS`。

采集员本人释放条目后，30 分钟内再次领取会原子跳过该条并继续查找同任务的下一条；其他采集员可以立即领取，管理员释放不触发原采集员冷却。该规则复用既有释放操作历史，不改变领取/释放接口，也不需要数据库迁移。
