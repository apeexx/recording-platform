...
| 领域 | 已实现能力 |
|---|---|
| Web 管理 | ADMIN/REVIEWER 登录与会话接管、角色路由、任务配置、数据池、CSV 导入、权限、审核、用户、操作记录和统计 |
| 小程序采集 | 微信/数字账号登录、“我的”直达资料与头像设置、任务授权、领取、录音、文本复制/补充、纯文本 JSON 与录音 multipart 提交、参考媒体时长自动检测、返修、提交后自动领取，以及独立“统计”页的任务汇总、每日分段和提交记录 |
| 任务与审核 | 草稿、运行、暂停、结束状态；文本/音频/视频参考；文字或录音成果；人工审核、驳回返修和状态并发控制 |
