...
当前仓库包含管理员/审核员 Web 端、原生微信小程序采集端、Java Spring Boot 后端和 MongoDB 数据层。平台已形成任务采集与人工审核闭环，并提供语音生成生产台和受限的外部脚本集成；机器审核和真实 AI 转写尚未实现。

## 项目定位

| 模块 | 当前入口 | 主要职责 |
|---|---|---|
| Web 管理端 | `apps/web/` | 真实数据大屏、后台身份、任务、可筛选数据池、授权、审核、用户、操作记录、统计和语音生成 |
| 微信小程序 | `apps/miniprogram/` | 采集员登录、资料、权限申请、任务领取、录音/文字提交、返修、结果查看和按任务统计 |
| Spring Boot 后端 | `backend/` | 身份与会话、任务状态、媒体、导入、审核、统计和外部集成 API |
| MongoDB | `recording_platform` | 用户、会话、任务、条目、媒体、导入、幂等和语音生成数据 |
...
| 领域 | 已实现能力 |
|---|---|
| Web 管理 | ADMIN/REVIEWER 登录与会话接管、角色路由、真实数据大屏、任务配置、状态/采集员/结果筛选数据池、条目详情、CSV 导入、权限、审核、用户、操作记录、跨页异步批处理，以及带任务、人员、日期筛选和采集员排名的统计 |
| 小程序采集 | 微信/数字账号登录、“我的”直达资料与头像设置、任务授权、领取、录音、文本复制/补充、纯文本 JSON 与录音 multipart 提交、参考媒体时长自动检测、返修、释放备注、标记无效与恢复，以及支持单日筛选的独立任务统计 |
| 任务与审核 | 草稿、运行、暂停、结束状态；固定展示文本/音频/视频三类参考源；文字或录音成果；人工审核、驳回返修、单页/跨页批量操作和状态并发控制 |
