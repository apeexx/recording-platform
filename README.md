...
| 领域 | 已实现能力 |
|---|---|
| Web 管理 | ADMIN/REVIEWER 登录与会话接管、角色路由、真实数据大屏、统一数字分页、任务配置、带脚本来源筛选和中文全量 CSV 导出的数据池、清空积压后仍保留真实汇总的双进度审核入口、审核池独立五维筛选与跨页批处理、任务级 AI 辅助审核，以及支持自动查询、悬停范围预览、三态排序、24 小时分布和独立人员详情的双阶段采集员统计 |
