...
| 领域 | 已实现能力 |
|---|---|
| Web 管理 | ADMIN/REVIEWER 登录与会话接管、角色路由、带服务端生成时间的真实数据大屏、ADMIN 邀请码创建/脱敏查看/永久停用、用户邀请码准入来源展示、任务配置、带脚本来源筛选和全量 CSV 导出的数据池、审核池五维筛选、只读原始结果与独立最终答案、任务级 AI 辅助审核、CSV 导入、三目录分页权限管理、用户、操作记录、跨页异步批处理，以及按任务、日期、人员下钻的采集员统计 |
