...
| 领域 | 已实现能力 |
|---|---|
| Web 管理 | ADMIN/REVIEWER 登录与会话接管、角色路由、真实数据大屏、ADMIN 邀请码创建/脱敏查看/永久停用、用户邀请码准入来源展示、任务配置、数据池行内快捷操作、审核池五维筛选、只读原始结果与独立最终答案、任务级 AI 辅助审核、CSV 导入、三目录分页权限管理、用户、操作记录、跨页异步批处理，以及带任务、人员、日期筛选和采集员排名的统计 |
...
任务条目编号以任务持久化 `itemSequence` 为主。每次有效创建都会先读取该任务已有条目的最大 `sequence`，再由 MongoDB 原子分配“当前计数器与该最大值中较大者加一”；历史导入或迁移使计数器落后时会自动抬高，计数器更高或保存失败造成的编号间隔均不会回退或复用。
