...
Web 表单校验、按钮操作、搜索、刷新及接口失败统一使用右上角 Toast；错误提示约 4.5 秒并自动去重相同文案，成功和信息提示约 2.6 秒。操作失败不会再覆盖当前页面或重复显示行内红字，弹窗提交失败会保留输入内容；已有数据刷新失败时保留旧数据。只有页面核心数据首次加载完全失败才显示带重试入口的阻塞状态，删除、结束、接管等确认交互保持原确认弹窗。
...
- 小程序仅面向重庆砚数科技有限公司内部员工及经授权的录音采集人员。登录首页在微信和数字账号登录操作之前常驻展示使用范围、录音任务用途和未受邀人员提示；该审核说明不增加游客模式，也不改变现有登录、会话接管、权限或个人信息处理流程。
...
- 标注脚本中心通过专用机器端点 `POST /api/integrations/tasks/{taskId}/items` 或 `POST /api/integrations/tasks/by-code/{taskCode}/items` 添加单条任务数据，并通过 `GET /api/integrations/items/{itemId}`、`GET /api/integrations/items/{itemId}/audio` 读取完成结果。四个端点都要求 `X-API-Key`，写入另要求 `Idempotency-Key`；Web Cookie、CSRF 或小程序 Bearer 均不能替代机器 Key。机器凭证不能访问管理员、审核或其他 `/api/**`。
...
- 外部脚本集成：两个写入入口分别按内部 `taskId` 和可见 `taskCode` 定位任务；JSON 可包含 `referenceText`、`referenceAudioUrl`、`referenceVideoUrl` 任意非空组合，并可选成对携带 `sourcePlatform`、`sourceItemId`。来源绑定在同一任务内唯一，重复来源返回 `409 SOURCE_ITEM_ALREADY_BOUND`，不同任务允许相同来源；普通后台添加和 CSV 导入不设置这两个字段。写入成功返回 `{ itemId, taskId, itemCode, status, createdAt }`，并继续复用任务状态、启用参考类型、HTTPS URL、序号和幂等规则；录音平台只保存 URL，不请求、下载或代理外部媒体。`GET /api/integrations/items/{itemId}` 只有 `COMPLETED` 才返回当前结果；音频读取端点继续只代理完成结果的当前录音。
...
- 启用人工审核时，驳回使用任务配置的原因多选加补充说明并进入原采集员的返修队列；通过时文本成果允许补改文字但不强制补充，录音文件保持不变并进入已完成。
...
- 任务配置、池数据、采集权限申请审批、领取、MP3 录音试听/上传通过。2026-07-22 起 `TEXT` 任务重新支持纯文本、纯录音或两者同时提交。
