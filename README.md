...
| 领域 | 已实现能力 |
|---|---|
| Web 管理 | ADMIN/REVIEWER 登录与会话接管、角色路由、真实数据大屏、统一数字分页、任务配置、可从详情返回恢复筛选和分页且长期记忆所选任务的数据池、中文全量 CSV 导出、双进度审核入口、审核池独立五维筛选与跨页批处理、审核领取后的标记无效、任务级 AI 辅助审核，以及列表与人员详情均可快捷切换日期的双阶段采集员统计 |
...
| 任务与审核 | 草稿、运行、暂停、结束状态；固定展示文本/音频/视频三类参考源；文字或录音成果；人工审核、驳回返修、审核标记无效与软恢复、单页/跨页批量操作和状态并发控制；AI 音频转写单条上限 20MB |
