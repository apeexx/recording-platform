...
管理员端当前允许维护除“语音生成”预留模块外的静态前端原型页面。静态原型只能使用脱敏示例数据和本地交互，不调用接口、不落盘、不定义数据库集合，也不得让文案暗示平台已经具备真实生产业务能力。
...
管理员侧边栏大分类必须支持多组同时展开，不允许实现为只能展开一个分类的手风琴模式。带 `children` 的大分类点击只负责展开/收缩，不直接跳转；子菜单负责页面跳转。

管理员侧边栏二级菜单折叠动画优先使用 CSS transition 实现。维护该交互时，二级菜单 DOM 应保持常驻，通过 class 切换控制展开和收起状态，不要为了动画引入重量级动画库。

`apps/web/src/pages/admin/voice-generation/` 是合作者预留模块。非语音生成任务不要修改该目录下的页面，除非用户明确要求。

语音生成模块当前只保留占位页面，不实现语音生成业务逻辑，不调用接口，不写真实业务数据。

除语音生成外的管理员端静态原型页面可以复用 `apps/web/src/components/admin/AdminPrototypePage.vue` 和 `apps/web/src/data/adminStaticData.js`。这些页面只用于展示管理后台布局、筛选、标签页、表格详情、状态切换和模拟操作提示，不代表已实现真实任务、审核、导出、权限或系统配置能力。
...
涉及启动方式、依赖安装、环境要求、验证命令变化时，必须同步更新 `scripts/README.md`。
