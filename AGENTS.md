...
后台登录页为 `/login`，首次改密页为 `/first-password`。所有 Web API 调用应优先复用 `apps/web/src/lib/httpClient.js`，由该模块统一携带同源 Cookie、获取并回传 CSRF、解析统一错误和处理 `SESSION_REPLACED`；不得在业务模块另写一套绕过 CSRF 的 fetch。
...
当前已实现可配置的单层人工审核：审核员可跨任务查看审核池、单条或批量领取、释放占用、补改文本、通过或驳回；管理员可指定审核员并批量通过。驳回回到原采集员的待录制状态，提交和操作历史永久保留。
