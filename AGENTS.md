...
数据一致性要求：taskCode 使用 Mongo 原子序列生成且不复用；仅 DRAFT 可编辑，发布后定义永久冻结；TEXT 允许纯文本、纯录音或两者同时提交，AUDIO 只允许录音；时长范围固定 1–600 秒；关闭人工审核时 rejectionReasons 固定为空；写操作持久化幂等；aiEnabled 首期必须 false
