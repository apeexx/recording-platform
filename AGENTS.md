...
当前已允许实现身份、会话、后台用户管理、微信登录边界、语音生成持久化，以及平台、不可变任务版本、授权申请、任务池领取、录音提交/返修/释放、媒体读取和 CSV/XLSX 导入后端闭环；仍不实现机器审核状态机、完整审核流或对应业务页面。
...
当前后端提供身份、会话、后台用户管理、语音生成、平台、任务版本、授权、任务池、录音媒体和导入 API；尚不提供机器审核状态机或完整一审/二审业务 API。
