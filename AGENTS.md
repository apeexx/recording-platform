...
仓库内不再维护 Docker Compose 配置。当前语音生成测试不需要本地数据库；后续需要本地 PostgreSQL 时，由开发者在本机或外部环境自行提供，并在具体任务中说明连接方式、风险和验证方式。
...
两个窗口标题分别为 `Recording Backend` 和 `Recording Frontend`，用于查看实时日志。脚本只负责启动后端和 Web 前端，不启动数据库，不创建 `.env`，不写入或打印 API Key，不再创建或写入根目录 `logs/`。语音生成真实联调仍需根目录 `.env` 提供 MiniMax 配置。
...
付费克隆模式只允许上传母带音频并填写新音色 ID，不展示、不接收、不提交语速、音量或语调参数。MiniMax 克隆母带要求为 mp3、m4a 或 wav，时长 10 秒到 5 分钟，文件不超过 20MB；新音色 ID 需要以英文字母开头，只包含字母、数字、下划线或连字符，长度为 8 到 256 个字符，且不能以下划线或连字符结尾。后端 multipart 上传上限配置为单文件 20MB、完整请求 21MB，超过限制时返回 HTTP 413 和脱敏错误摘要。

当前语音生成合成、试听、克隆、记录查询和默认配置保存不做数据库持久化。生成记录和默认声音配置只保存在后端进程内存中，后端重启后会清空；生成音频文件仍保存到本地目录，用于当前会话内播放和下载。默认生成目录 `backend/storage/voice-generation/` 是本地运行产物，必须保持 Git 忽略，不得提交。后续恢复持久化时，正式数据库方向为 PostgreSQL。
...
数据一致性要求：成功后写入后端内存记录，并将音频保存到本地存储目录；后端重启后记录清空
...
权限要求：当前阶段不接入登录/JWT；语音生成接口为本地开发联调用途
数据一致性要求：后端先上传母带获取 MiniMax file_id，并以数值类型提交克隆请求；成功后写入一条 CLONE 类型内存记录；后端重启后记录清空
前端调用位置：apps/web/src/lib/voiceGenerationApi.js
```
...
错误码：400 查询失败
权限要求：当前阶段不接入登录/JWT；语音生成接口为本地开发联调用途
数据一致性要求：从后端内存记录按创建时间倒序分页读取；后端重启后记录清空
前端调用位置：apps/web/src/lib/voiceGenerationApi.js
```
...
错误码：400 参数缺失或保存失败
权限要求：当前阶段不接入登录/JWT；语音生成接口为本地开发联调用途
数据一致性要求：写入或读取后端内存中的默认配置；后端重启后恢复默认值
前端调用位置：apps/web/src/lib/voiceGenerationApi.js
```
...
当前阶段暂不定义正式业务表、字段、索引或迁移脚本。语音生成模块为便于先测试生成和下载功能，临时使用后端进程内存保存生成记录和默认声音配置，不提供持久化保证。

后续用户、角色、任务、任务领取、录音文件、审核记录、状态流转和操作日志均以 PostgreSQL 关系模型设计。需要状态一致性的流程优先使用事务、唯一约束、外键、索引和状态日志表；半结构化第三方返回摘要可使用 `jsonb` 字段，但核心业务字段必须结构化。
