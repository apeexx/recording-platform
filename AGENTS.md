...
- 数据库：PostgreSQL 作为项目默认关系型数据库
- 文档记录：根目录 `README.md`、`AGENTS.md`、`log.md`

仓库内不再维护 Docker Compose 配置。需要本地 PostgreSQL 时，由开发者在本机或外部环境自行提供，并在具体任务中说明连接方式、风险和验证方式。
...
两个窗口标题分别为 `Recording Backend` 和 `Recording Frontend`，用于查看实时日志。脚本只负责启动后端和 Web 前端，不启动 PostgreSQL，不创建 `.env`，不写入或打印 API Key，不再创建或写入根目录 `logs/`。语音生成真实联调仍需开发者自行准备 PostgreSQL 和根目录 `.env`。
...
语音生成合成、试听、克隆、记录查询和默认配置保存依赖 PostgreSQL。后端遇到数据库不可用时必须返回脱敏 JSON 错误摘要，不得让数据库异常穿透为 Internal Server Error。日常合成应先确认生成记录可写入，再调用 MiniMax 远程合成，避免数据库不可用时产生不必要的远程调用。
...
当前阶段除语音生成模块外，暂不定义正式业务表、字段、索引或迁移脚本。

语音生成 PostgreSQL 表说明：

```text
表名称：voice_generation_records
...
唯一约束：id 主键
索引：created_at 倒序索引，status、mode、voice_id 普通索引
数据兼容策略：音频二进制不入库，只保存本地文件路径和元数据
迁移步骤：由 Flyway `V1__create_voice_generation_tables.sql` 创建
回滚方式：停止语音生成接口后可按需备份并删除该表和本地音频目录
```

```text
表名称：voice_generation_configs
