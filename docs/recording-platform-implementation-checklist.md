...
- [x] **Step 1.4：实现逐条审核和 ADMIN 批量通过。**
  ...
  REVIEWER 审核池只显示未占用条目，必须领取并只能处理本人审核占用；ADMIN 审核池显示全部待审核条目，可直接单条通过/驳回，但不显示审核领取和释放入口。CSRF 失效与角色越权分别返回 `CSRF_TOKEN_INVALID`、`ACCESS_DENIED`。
...
- [x] **Step 4.3：实现审核池和工作台。**
  ...
  REVIEWER 随机/批量领取并释放本人占用；ADMIN 分配、直接单条审核或批量通过；工作台展示参考媒体、采集音频、补改文字、逐条通过、预设多选+补充说明驳回，并按角色隐藏不适用入口。
...
- [x] **Step 5.6：Node、结构和开发者工具验证。**
  ...
  初始交付时已完成 Node 纯逻辑、JavaScript 语法和 JSON/页面路径静态检查，微信开发者工具 CLI 因 IDE 服务端口未开启停在人工确认边界。2026-07-15 已补做开发者工具与真机联调：完成微信登录、姓名、授权申请、领取、MP3 录音试听/上传、文字单独提交、返修重录覆盖、释放清理、审核结果回显和个人统计核对。
...
- [x] **Step 6.1：写 Mongo/存储就绪红灯测试。**

  健康接口只返回总体、Mongo 和目录可写状态，不返回 URI、密码、绝对路径或异常堆栈。

- [x] **Step 6.2：改启动脚本做脱敏前置检查。**
...
- [x] **Step 6.5：浏览器和微信验收。**
  ...
  初始自动化验收使用 Edge/Playwright 复验登录页桌面与窄屏布局，并确认后端不可达时显示脱敏 HTTP 502 失败态。2026-07-14 至 2026-07-15 已在本机 MongoDB、真实浏览器、微信开发者工具及真机环境补做完整联调：后台登录接管、首次改密、角色菜单、平台/任务/版本、权限申请审批、池数据、录音与文字提交、返修、释放、废弃恢复、管理员与审核员人工审核、媒体 Range 播放、操作记录以及任务/采集员/审核员/小程序个人统计均通过。付费 MiniMax 调用仍未执行。
...
- [x] 后端身份、任务、采集、审核、返修、废弃、操作记录和统计全部有 API 与自动化测试。
- [x] Web 不再依赖静态假按钮，ADMIN/REVIEWER 有真实路由、权限、加载/空/失败态。
- [x] 小程序可完成微信登录、姓名、申请、领取、录音/文字、提交、释放、返修、自动下一条和个人统计。
- [x] 所有业务 mutation 具备 operationId/Idempotency-Key 和 revision/CAS 边界。
- [x] Mongo 索引、媒体目录、导入 lease、cleanup job、SSRF 和会话安全均有验证记录。
- [x] `README.md`、`AGENTS.md`、模块 README 和 `log.md` 与真实代码一致。
- [x] 全量测试、Web build、敏感扫描和 `git diff --check` 通过。
- [x] 真实 MongoDB 与微信真机联调已补充完成；付费 MiniMax 调用继续明确记录为未执行。
...
- [x] 迁移器改为所有路径与 MongoDB 快照全量预检后再提交，后续路径失败时全局逆向补偿已完成路径。
- [x] MongoDB 正向与回滚均使用版本号和精确旧/新路径 CAS；每次替换推进版本，避免覆盖并发修改。
- [x] 回滚 CAS 冲突或异常时保留新路径文件并补回旧路径，使两类引用继续可读。
- [x] README 与长期规则已记录默认关闭、停写备份、单实例执行、二次零迁移复检、关闭开关和失败恢复步骤。
- [x] 微信真机返修、同条目重录覆盖与释放清理已完成人工验收：同一条目返修后稳定文件原子覆盖为最新 MP3，审核页可播放；采集员释放后条目回到 `AVAILABLE`，当前结果与稳定文件删除，提交及操作历史保留。
- [x] 真实角色与统计闭环已完成人工验收：管理员直接审核、审核员领取/释放/再次领取/补文通过均成功；任务、采集员、审核员和小程序个人统计与操作历史一致。
